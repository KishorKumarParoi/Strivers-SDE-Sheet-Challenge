...
Date: 2024-06-25
Time: 15:14:35
Problem: Remove Duplicates from Doubly Linked List
...
/**
 * The function `removeDuplicates` in C++ removes duplicate nodes from a doubly linked list.
 *
 * @param head The `head` parameter in the provided code represents the head of a doubly linked list
 * containing integer values. The function `removeDuplicates` aims to remove any duplicate elements
 * from the linked list while maintaining the order of non-duplicate elements.
 *
 * @return The function `removeDuplicates` is returning a pointer to the head of the linked list after
 * removing any duplicate nodes.
 */
Node<int>* removeDuplicates(Node<int>* head) {
    ...
    Node<int>* temp = head;
    Node<int>* prevNode = new Node(-1LL);

    while (temp != NULL) {
        // if (temp->next != NULL)
            // cout << temp->data << " " << temp->next->data << endl;

        if (temp->next != NULL and temp->next->data == temp->data) {
            // cout << temp->data << " " << temp->next->data << endl;
            Node<int>* delNode = temp;
            temp = temp->next;
            delete delNode;
        }
        else {
            ...
            if (temp->next == NULL)
                break;
            temp = temp->next;
            // cout << "prev: " << prevNode->data << " " << temp->data << endl;
        }
    }
